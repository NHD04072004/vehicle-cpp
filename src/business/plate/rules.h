...
// Định nghĩa thật nằm ở business/violation/kinds.h (đó là khái niệm VI PHẠM,
// không phải luật biển số). Re-export để mã gọi qua `plate::EventKind` vẫn chạy.
using violation::EventKind;
using violation::EventKindMask;
using violation::ViolationState;
using violation::eventKindCode;
using violation::eventKindName;
using violation::kEventKindCount;
using violation::kindBit;
using violation::kindIndex;
using violation::maskHas;
